...
#import "DPCPickerColumnView.h"

@interface DPCPickerView()<DPCPickerColumnViewDelegate>
...
@property (nonatomic, strong) NSArray<DPCPickerColumnView *> *columnViewList;
...
- (void)setup {
    ...
    self.textFontOfSelectedRow = [UIFont systemFontOfSize:18 weight:UIFontWeightMedium];
    self.textColorOfOtherRow = [UIColor lightGrayColor];
    self.textFontOfOtherRow = [UIFont systemFontOfSize:14 weight:UIFontWeightRegular];
...
- (DPCPickerColumnView *)createColumnViewAtComponent:(NSUInteger)component refresh:(BOOL)refresh {
    ...
    DPCPickerColumnView *view = [self columnViewInComponent:component];
    if (!view) {
        ...
        view = [[DPCPickerColumnView alloc]initWithFrame:CGRectMake(component * width, 0, width, kHeight) rowHeight: rowHeight upLineHeight:upLineHeight downLineHeight:downLineHeight isCycleScroll:self.isCycleScroll datas:datas];
        view.delegate = self;
...
- (void)setupColumnView {
    ...
    for (int i = 0; i < _numberOfComponents; i++) {
        DPCPickerColumnView *view = [self createColumnViewAtComponent:i refresh:false];
...
- (void)setupView {
    ...
    switch (self.type) {
        case DPCPickerViewLineTypeline:
        {
            [self setupLineView1];
            return;
        }
        case DPCPickerViewLineTypelineSegment:
        {
            [self setupLineView2];
            return;
        }
        case DPCPickerViewLineTypelineVertical:
        {
            [self setupLineView3];
            return;
        }
...
- (void)setupLineView2 {
    ...
    if (_numberOfComponents == 1) {
        ...
        upLineHeight = upLineHeight > 0.5 ? upLineHeight: 1;
        CGFloat upLinePosY = kHeight / 2 - rowHeight / 2 - upLineHeight;
        UIView *upLine = [[UIView alloc]initWithFrame:CGRectMake(16, upLinePosY, self.frame.size.width-32, upLineHeight)];
        upLine.backgroundColor = self.lineBackgroundColor;
        [self addSubview:upLine];
        [upLines addObject:upLine];
        
        CGFloat downLineHeight = [self downLineHeightForComponent:0];
        downLineHeight = downLineHeight > 0.5 ? downLineHeight: 1;
        CGFloat downLinePosY = CGRectGetMaxY(upLine.frame) + rowHeight;
        UIView *downLine = [[UIView alloc]initWithFrame:CGRectMake(16, downLinePosY, self.frame.size.width-32, downLineHeight)];
        downLine.backgroundColor = self.lineBackgroundColor;
        [self addSubview:downLine];
        [downLines addObject:downLine];
...
- (DPCPickerColumnView *)columnViewInComponent:(NSUInteger)component {
    if (!self.columnViewList || self.columnViewList.count == 0) {
        return nil;
    }
    __block DPCPickerColumnView *view = nil;
    [self.columnViewList enumerateObjectsUsingBlock:^(DPCPickerColumnView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
#pragma mark - DPCPickerColumnViewDelegate
...
#pragma mark - Setter
- (void)setDataSource:(id<DPCPickerViewDataSource>)dataSource {
...
- (void)setTextColorOfSelectedRow:(UIColor *)textColorOfSelectedRow {
    ...
    [self.columnViewList enumerateObjectsUsingBlock:^(DPCPickerColumnView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        obj.textFontOfSelectedRow = [UIFont systemFontOfSize:28 weight:UIFontWeightMedium];
    }];
}
...
- (void)setTextFontOfSelectedRow:(UIFont *)textFontOfSelectedRow {
    ...
    [self.columnViewList enumerateObjectsUsingBlock:^(DPCPickerColumnView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.textFontOfSelectedRow = textFontOfSelectedRow;
    }];
}
...
- (void)setTextFontOfOtherRow:(UIFont *)textFontOfOtherRow {
    ...
    [self.columnViewList enumerateObjectsUsingBlock:^(DPCPickerColumnView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.textFontOfOtherRow = textFontOfOtherRow;
    }];
}
